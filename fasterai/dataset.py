...
#Note- avoiding other augmentations as default besides random flip, because lighthing changes, rotions, etc, adversely
#affect the target image (bluring, altering colors, etc) in the case of image modification/generation training.
...
def get_matched_image_model_data(image_size: int, batch_size: int, root_data_path: Path, train_root_path: Path, proj_id: str, 
    ...
    tfms = tfms_from_stats(inception_stats, image_size, tfm_y=TfmType.PIXEL, aug_tfms=aug_tfms, sz_y=image_size)
